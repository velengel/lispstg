...
(defgeneric gc-enemies (enemy-manager game-field)
            (:documentation "delete unnecessary enemies"))

(defmethod gc-enemies ((enemy-manager enemy-manager) (game-field game-field))
  (with-slots (enemies) enemy-manager
    (setf enemies
          (delete-if #'(lambda (enemy)
                         (out-of-field-p enemy game-field))
                     enemies))))
